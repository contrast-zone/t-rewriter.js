...
In a **symbolic approach** that Tricosm is taking to produce outputs from inputs, there is an analogy in which the **training processes in ANN-s is being replaced by algorithm synthesis in Tricosm**. It is possible to feed to Tricosm a formula in a form of `f (program (input) -> output) -> program` where function `program` is being automatically constructed and returned by higher order function `f`, provided that we know what `input -> output` mappings hold. Advantages of this approach over training ANN-s may be significantly shorter processing time, as well as possibility of adjusting a percent of certainity up to 100% in constructing valid outputs from inputs.
...
We are taking a stand that fields of using Tricosm is actually inexhaustible area, considering all the possibilities of program synthesis. Nevertheless, the **metacompiling** and **theorem proving** areas seem to be reasonable application fields from an aspect of attracting actual users. If you have ideas about other reasonable applications to attract users, please join the [Tricosm discussion area](https://github.com/tricosm/tricosm-core/discussions).
