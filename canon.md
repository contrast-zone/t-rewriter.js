...
# canon: rule based programming formalism
...
1. the rules may be written in a forward or backward manner:  
    Forward rules are analogous to conjunction of [material implications](https://en.wikipedia.org/wiki/Material_conditional), while backward rules are analogous to disjunction of [converse nonimplications](https://en.wikipedia.org/wiki/Converse_nonimplication). In our understanding, we hold that these rules correspond to [deduction](https://en.wikipedia.org/wiki/Deductive_reasoning) and [abduction](https://en.wikipedia.org/wiki/Abductive_reasoning), respectively. This combination of rules is chosen because of their symetric property, such that algorithms for processing one type of rules works with the other type of rules in a similar manner, only applied backwards. Both kinds of rules, as it will be explained, are combined to restrain possibly infinite production rule divergence between starting input and final output terms;
...
Most generally speaking, *Canon* may be used to express a wide variety of languages. Different languages may be used to express a wide variety of systems. Different systems, in turn, may be used to express a wide variety of processes we experience around us. Being natural or artificial, many of these processes may deserve our attention while understanding and mastering them may be of certain importance to us. All these processes may be reasoned about using different formal languages, while *Canon*'s pupose is to simplify description of such languages.

To describe a range of formal languages *Canon* is aiming at, we are unlocking the doors of no less than a general computing platform. This entails being able to describe any computation kinds known to us. Such requirement of universality finally raises a bar of potential use cases of *Canon*. Thus, what will *Canon* represent, and where it will be used depends only on our imagination because with a kind of systems like *Canon*, we are entering a nonexhaustive area of general knowledge computing.

However, suitability of using *Canon* for particular purposes may be determined by *Cannon* appearance. Being Turing complete, which is generally not so hard to achieve, doesn't necessarily implies being a good choice for particular use. What *Cannon* is really trying to be good at is a convenient syntax/semantic declaration of various languages. In a light of somewhat innovative approach using forward/backward semantic rhombus mnemonics, the search space that relates input to output expressions may reduce to a finite amount of production steps. And this is the approach that distincts *Canon* from similar solutions to the same recurring problem: [metaprogramming](https://en.wikipedia.org/wiki/Metaprogramming).
