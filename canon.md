...
However, suitability of using *Canon* for particular purposes may be determined by *Cannon* appearance. Being Turing complete, which is generally not so hard to achieve, doesn't necessarily imply being a good choice for particular use. What *Cannon* is really trying to be good at is a convenient syntactic/semantic declaration of various languages. Thus, in a light of somewhat innovative approach using forward/backward semantic rhombus mnemonics, *Canon* tries to provide a solution to the recurring problem of syntax and semantics agnostic [metaprogramming](https://en.wikipedia.org/wiki/Metaprogramming).
