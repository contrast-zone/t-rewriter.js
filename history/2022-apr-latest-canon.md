...
To describe a range of formal languages *Canon* is aiming at, we are unlocking the doors of no less than a general computing platform. This entails being able to describe any computation kinds known to us. Such requirement of universality finally raises a bar of potential use cases of *Canon*.

However, being Turing complete, which is generally not so hard to achieve, doesn't necessarily imply being a good choice for a particular use. What *Cannon* is really trying to be good at is a convenient syntactic/semantic declaration of various languages. Thus, in a light of somewhat innovative approach using forward/backward semantic rhombus mnemonics, *Canon* tries to provide a solution to the recurring problem of syntax and semantics agnostic [metaprogramming](https://en.wikipedia.org/wiki/Metaprogramming).
