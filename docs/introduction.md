...
# Introduction to *expression logic* formal language and its implementation
...
2. **v-parse-plus** algorithm extends the original algorithm to handle right side sequences of atoms, together with *identifier equivalence* operator.
3. **v-parse-star** algorithm finally extends the previous one to handle left side conjunctions and right side disjunctions.

Version (1) is the simplest one, and may be compared to the current mainstream status of parsing technology. Version (2) with its extensions already brings in the Turing completeness. Version (3) is the final algorithm version, supporting interpretation of normalized sequents. Although one may argue that version (3) would not be unconditionally necessary because version (2) is already Turing complete, we are taking the stand that the sequent calculus extensions greatly simplify formalization of different systems, and we decide to keep it as a core part of the inference system. Moreover, original author's impression is that implication connective introduced already in version (1) simply asks to be completed with conjunction and disjunction connectives from sequent calculus, thus forming a consistent whole.
